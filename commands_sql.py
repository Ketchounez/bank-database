...
payments_command_fourth = """
    with temp as (
            SELECT credit_id, sum(payment_amount) as overall_pay
            FROM payments
            GROUP BY credit_id
    ),
    craq as (
        select cl.client_id, cl.first_name, cl.last_name, cr.credit_id, DATE_ADD(crco.contract_date, INTERVAL cr.loan_term DAY)  as to_date, cr.loan_amount, t.overall_pay
        FROM credits as cr
        LEFT JOIN temp as t on cr.credit_id = t.credit_id
        LEFT JOIN clients_credits as cc on cr.credit_id = cc.credit_id
        LEFT JOIN clients as cl on cc.client_id = cl.client_id
        left join credit_contracts as crco on cr.credit_id = crco.credit_id
        order by cl.client_id
    ),
     lq as (
        select cq.client_id, cq.first_name, cq.last_name, cq.credit_id, cq.loan_amount, cq.overall_pay, datediff(cq.to_date, curdate()) as remained_days, (cq.overall_pay * 100 / cq.loan_amount) as paid
        from craq as cq
     )
     
     select client_id, first_name, last_name, loan_amount,
        if (overall_pay = loan_amount, remained_days = 0, remained_days) as remained_days, paid
     from lq
    """
